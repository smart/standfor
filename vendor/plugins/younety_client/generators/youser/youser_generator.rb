class YouserGenerator < Rails::Generator::NamedBase
  ...
  def manifest
    recorded_session = record do |m|
      ...
      m.template 'youser_authenticator.rb', File.join('app/models', "youser_authenticator.rb")
            
      if options[:include_activation]
        %w( notifier observer ).each do |model_type|
          m.template "#{model_type}.rb", File.join('app/models', class_path, "#{file_name}_#{model_type}.rb")
        end
      end
      ...
      unless options[:skip_migration]
        ...
        m.migration_template 'authenticator_migration.rb', 'db/migrate', :assigns => {
          :migration_name => "CreateYouserAuthenticators"
          }, :migration_file_name => "create_youser_authenticators"
      end
    end
