class AdisController < ApplicationController
  ...
  def render_adi
    ...
    adi_response = Net::HTTP.get_response(URI.parse(ADISERVER + "/adis/#{@my_badge.adi_id}.#{params[:ext]}"))
    send_data adi_response.body, :filename => params[:action] + "#{params[:ext]}", :type =>"image/#{params[:ext]}", :disposition => 'inline'
