module OpenIdHelper
  ...
  private
    def successful_openid_login(identity_url, registration = {})
      unless @account = Account.find_by_youser(identity_url, "OpenId")
        @account = Account.new(:create_auth_token => identity_url, :create_authenticator => "OpenId")
    ...
    def assign_open_id_attributes(identity_url, registration)
      { 
        ...
        unless registration[registration_attribute].blank?
          @account.send("#{model_attribute}=", registration[registration_attribute])
        end
      end
